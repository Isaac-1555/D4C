...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx d4c-ai <command> [provider]
...
Examples:
  npx d4c-ai login              # interactive provider selection
  npx d4c-ai login anthropic    # login to specific provider
  npx d4c-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx d4c-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx d4c-ai --help' for usage`);
	process.exit(1);
}
